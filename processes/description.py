...
def generate_video_title(surah: Surah, reciter: Reciter, has_translation: bool, filename: str, is_short: bool = False, custom_title: str = None, start: int = 1, end: int = 1):
    ...
    if is_short:
        ...
        title_str += f"{surah.bangla_name} - {e2b(str(start))}"
        if start != end:
        ...
        title_str = f"{e2b(str(surah.number))} - সুরাহ {surah.bangla_name} - {reciter.bangla_name}{' - বাংলা অনুবাদসহ' if has_translation else ''}"
...
def generate_video_description(surah: Surah, reciter: Reciter, filename: str, is_short: bool = False, start: int = 1, end: int = 1):
    with open(filename, "a", encoding="utf-8") as f:
        if is_short:
            f.write(f"সুরাহ {surah.bangla_name} (আয়াত {e2b(str(start))}-{e2b(str(end))})\n")
            ...
            f.write(f"""{e2b(str(surah.number))} - সুরাহ {surah.bangla_name}\nতিলাওয়াতঃ {reciter.bangla_name}\n\n""")
        ...
        tags = ["Quran", "Islam", "Islamic", "Quran Tilawat", reciter.english_name, surah.english_name, "Takwa Bangla"]
        if is_short:
