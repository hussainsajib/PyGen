...
def calculate_juz_offsets(surahs_in_juz: list, surah_durations: dict, bsml_duration: float, silence_duration: float = 0.0):
    """
    Calculates the timing offset for each surah in a Juz relative to the start of the Juz.
    Account for Basmallah duration, configurable silence, and Surah 9 (At-Tawbah) gap.
    """
    offsets = {}
    current_cumulative_time = 0.0
    
    for i, surah_num in enumerate(surahs_in_juz):
        # Determine injection duration for this surah
        injection_time = 0.0
        if surah_num == 9:
            # Surah At-Tawbah exception: 5s silence gap instead of Basmallah
            injection_time = 5.0
        elif surah_num != 1:
            # All other Surahs except Al-Fatihah need Basmallah + Silence
            injection_time = bsml_duration + silence_duration
            
        # The offset for the current surah starts after its injection
        offsets[surah_num] = current_cumulative_time + injection_time
        
        # Advance cumulative time by injection + surah duration for the next one
        current_cumulative_time += injection_time + surah_durations.get(surah_num, 0.0)
