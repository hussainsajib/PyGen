...
def mock_context():
    with (patch("processes.surah_video.CompositeVideoClip") as mock_composite,
        ...
        mock_clip.to_ImageClip.return_value = mock_clip
...
@pytest.mark.asyncio
async def test_wbw_full_translation_source_respected(mock_context):
    ...
    # 1. Test create_ayah_clip
    def config_get_side_effect(key, default=None):
        if key == "WBW_FULL_TRANSLATION_ENABLED": return "True"
        if key == "WBW_FULL_TRANSLATION_SOURCE": return "custom_source"
        return default
    mock_context["mock_config"].get.side_effect = config_get_side_effect

    create_ayah_clip(surah, ayah, reciter, 0, 5000, surah_data, translation_data, full_audio, False)
    # Verify that get_full_translation_for_ayah was called with "custom_source"
    mock_context["mock_get_full_trans"].assert_called_with(1, 1, "custom_source", language="bengali")

    # 2. Test create_wbw_ayah_clip
    mock_context["mock_get_full_trans"].reset_mock()
    create_wbw_ayah_clip(surah, ayah, reciter, 0, 5000, surah_data, translation_data, full_audio, False, segments)
    mock_context["mock_get_full_trans"].assert_called_with(1, 1, "custom_source", language="bengali")

    # 3. Test create_wbw_advanced_ayah_clip
    mock_context["mock_get_full_trans"].reset_mock()
    mock_final_clip = MagicMock()
    mock_final_clip.duration = 5.0
    mock_context["mock_concat"].return_value = mock_final_clip
    
    create_wbw_advanced_ayah_clip(surah, ayah, reciter, full_audio, False, segments, full_translation_db="fallback_db")
    mock_context["mock_get_full_trans"].assert_called_with(1, 1, "custom_source", language="bengali")

    # 4. Test fallback if config is None for create_wbw_advanced_ayah_clip
    def config_get_side_effect_none(key, default=None):
        if key == "WBW_FULL_TRANSLATION_ENABLED": return "True"
        if key == "WBW_FULL_TRANSLATION_SOURCE": return None
        return default
    mock_context["mock_config"].get.side_effect = config_get_side_effect_none
    
    mock_context["mock_get_full_trans"].reset_mock()
    create_wbw_advanced_ayah_clip(surah, ayah, reciter, full_audio, False, segments, full_translation_db="fallback_db")
    mock_context["mock_get_full_trans"].assert_called_with(1, 1, "fallback_db", language="bengali")

    # 5. Test fallback for create_ayah_clip
    mock_context["mock_get_full_trans"].reset_mock()
    # After implementation, we'll pass full_translation_db="fallback_db"
    create_ayah_clip(surah, ayah, reciter, 0, 5000, surah_data, translation_data, full_audio, False, full_translation_db="fallback_db")
    mock_context["mock_get_full_trans"].assert_called_with(1, 1, "fallback_db", language="bengali")

    # 6. Test fallback for create_wbw_ayah_clip
    mock_context["mock_get_full_trans"].reset_mock()
    create_wbw_ayah_clip(surah, ayah, reciter, 0, 5000, surah_data, translation_data, full_audio, False, segments, full_translation_db="fallback_db")
    mock_context["mock_get_full_trans"].assert_called_with(1, 1, "fallback_db", language="bengali")
